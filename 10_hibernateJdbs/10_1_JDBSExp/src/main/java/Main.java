...
ERROR: The server time zone value ‘ðйððð_ʱðð’ is unrecognized or represents more than one time zone. You must configure either the server or JDBC driver (via the serverTimezone configuration property) to use a more specific time zone value if you want to utilize time zone support.
Решение: Добавьте в вашу строку соединения с базой данных параметр, указывающий часовой пояс, serverTimezone=UTC или укажите конкретный часовой пояс, например serverTimezone=Europe/Moscow.
Ваша строка будет выглядеть так: jdbc:mysql://localhost:3306/skillbox?useSSL=false&serverTimezone=UTC

Критерии оценки
«Зачёт» — в консоль выводятся средние количества покупок курсов в месяц за 2018 год. «Незачёт» — задание не выполнено.
...
public class Main {
    public static void main(String[] args) {
        ...
        try{
            ...
            ResultSet resultSet = statement.executeQuery("SELECT pl.course_name, COUNT(pl.course_name) / (MAX(MONTH(pl.subscription_date)) - (MIN(MONTH(pl.subscription_date))- 1)) as result\n" +
                    "FROM skillbox.PurchaseList pl GROUP by pl.course_name;");
            while (resultSet.next()){
                //String courseName = resultSet.getString("name");
                System.out.println(resultSet.getString("pl.course_name") + " Среднее значение: " + resultSet.getString("result"));
            }
        }
        catch (Exception ex) {
            ex.printStackTrace();
